...
if (isRedirecting) {
  ...
  <div className="flex flex-col p-6 bg-white rounded-lg shadow-lg relative">
    ...
    <div
      className="flex items-center cursor-pointer gap-5 text-3xl -mt-4 border-b-2 border-gray-300 pb-2 mb-4"
    ...
    <div className="flex">
      {/* Primera columna: Imagen, botón, estrellas */}
      <div className="w-1/3 flex flex-col items-center">
        ...
        />
        <button
          className="mt-4 bg-primary hover:bg-blue-950 text-white font-bold py-2 px-4 rounded"
          onClick={openModal}
        >
          Ver reseñas
        </button>
        <div className="mt-2">{renderEstrellas(calificacionPromedio)}</div>
      </div>

      {/* Segunda columna: Detalles del libro */}
      <div className="ml-6 flex flex-col flex-grow">
        <h2 className="text-2xl font-semibold">{titulo}</h2>
        <p className="text-lg font-medium">
          <strong>Autor:</strong>{" "}
          {autores.length > 0
            ? autores.map((autor) => autor.nombre).join(", ")
            : "Autor no disponible"}
        </p>
        <p className="text-md">
          <strong>Serie:</strong> {serie || "N/A"}
        </p>
        <p className="text-md">
          <strong>ISBN:</strong> {isbn}
        </p>
        <div className="text-md break-all">{renderSinopsis()}</div>
      ...
      {resenas.length === 0 ? (
        ...
        resenas
          ...
          .map((reseña, index) => (
            <div
              key={index}
              className="flex items-start mb-4 p-4 border border-gray-200 rounded-lg bg-gray-100 break-all"
            >
              <img
    ...
    {isModalOpen && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 break-all">
        <div className="bg-white rounded-lg p-4 w-[40%] max-h-[800px] overflow-hidden">
          ...
          {/* Modal content */}
          <div className="modal-container" style={{ maxHeight: "400px", overflowY: "auto" }}>
            {loadingReseñas ? (
