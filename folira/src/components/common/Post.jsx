  ...
  return (
    <div className='flex gap-2 items-start p-4 border-b border-blue-950'>
      <div className='avatar'>
        <Link to={`/profile/${postOwner._id && postOwner.nombre}`} className='w-8 rounded-full overflow-hidden'>
          <img src={postOwner.fotoPerfil || "/avatar-placeholder.png"} alt='Profile' />
        </Link>
      </div>
      <div className='flex flex-col flex-1'>
        <div className='flex gap-2 items-center'>
          <Link to={`/profile/${postOwner._id && postOwner.nombre} `} className='font-bold'>
            {postOwner.nombreCompleto}
          </Link>
          <span className='text-blue-950 flex gap-1 text-sm'>
            <Link to={`/profile/${postOwner._id && postOwner.nombre}`}>@{postOwner.nombre}</Link>
            <span>·</span>
