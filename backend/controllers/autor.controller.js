...
=======
    try {
        const { nombre, seudonimo, fechaNacimiento, pais, biografia, distinciones, generoLiterarioPreferido } = req.body;
        let { fotoAutor } = req.body;
...
=======
    try {
       ...
       if (fotoAutor) {
        if (autorExistente.fotoAutor) {
            // Eliminar la imagen anterior de Cloudinary
            await cloudinary.uploader.destroy(autorExistente.fotoAutor.split("/").pop().split(".")[0]);
        }
        // Subir la nueva imagen
        const uploadedResponse = await cloudinary.uploader.upload(fotoAutor);
        fotoAutor = uploadedResponse.secure_url;
